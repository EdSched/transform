...
async function showStudentInfoTeacher(name) {
  ...
  const html = `
    ...
    ` : ''}
    ${s.status==='graduated' ? `<div style="margin:10px 0"><button onclick='openAdmissionEntry(${JSON.stringify({school:(p.target_schools||'').split(/[、,，\/]/)[0]||'',faculty:s.faculty||'',dept:'',student:s.name||'',jp:s.japanese_score||'',en:s.english_score||'',major:s.major||'',enroll:s.target_enrollment||''}).replace(/'/g,"&#39;")})' style="font-size:11px;padding:5px 14px;border:1px solid var(--accent,#8b5cf6);color:var(--accent,#8b5cf6);background:none;border-radius:4px;cursor:pointer">📝 录入合格实绩</button></div>` : ''}
